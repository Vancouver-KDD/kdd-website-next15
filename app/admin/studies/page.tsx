...
import {deleteStudy, getStudies} from '@/firebase/actions/study.admin'
        ...
        <Card key={study.id} className={cn('w-full', study.draft && 'opacity-60')}>
              ...
              {(() => {
                ...
                if (study.isOngoing || isNaturallyOngoing) {
                  return (
                    <Chip
                      size="sm"
                      classNames={{base: 'bg-emerald-600', content: 'text-white font-medium'}}>
                      {t('chips.ongoing')}
                    </Chip>
                  )
                }
          ...
          <CardBody>
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" variant="flat" onPress={() => router.push(`/study/${study.id}`)}>
                View
              </Button>
            </div>
          </CardBody>
