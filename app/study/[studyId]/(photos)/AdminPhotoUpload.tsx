...
import {uploadStudyPhoto} from '@/cloudinary/actions.admin'
...
export default function AdminPhotoUpload({studyId, onUploaded}: AdminPhotoUploadProps) {
      ...
      while (true) {
        ...
        try {
          ...
          const baseName = next.name.includes('.')
            ...
            : next.name
          const result = await uploadStudyPhoto({
            token: idToken,
            studyId,
            imageData: dataUrl,
            fileName: baseName,
  ...
  }, [studyId, onUploaded, updateItems, user])
