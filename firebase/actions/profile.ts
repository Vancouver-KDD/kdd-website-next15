...
import {firestore, storage, auth} from '@/firebase/server'
...
export async function updateUserMetadata(uid: string, displayName: string, photoURL: string, occupation?: string) {
  try {
...
export async function getUserActivity(uid: string) {
  try {
    ...
    const myComments: Comment[] = myCommentsSnap.docs.map(doc => {
      const data = doc.data()
      return {
         id: doc.id,
         targetId: data.targetId,
    ...
    const reactedComments = Array.from(reactedCommentsMap.values()).sort((a,b) => b.createdAt - a.createdAt)
